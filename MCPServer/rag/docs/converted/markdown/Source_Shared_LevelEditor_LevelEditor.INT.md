...
<!-- Source: Source_Shared_LevelEditor_LevelEditor.INT.udn -->
...
### Outliner
The Outliner lists all actors in the level by name, organized in a tree view. You can select actors in this list exactly as you would in the Viewport, and right-click them to access the same contextual actions.

You can also:

-	Filter the list to a subset of one or more actors you’re interested in. Use the filter dropdown to the left of the Search box to choose from a set of pre-made filters, or enter your own custom filter text in the Search box. (You can also create new filters from your custom searches.)

-	View details about the listed actors in dedicated columns, such as their visibility in editor, modified status, and source control status. Right-click the header row to choose the details you want to see.

-	Create folders to group related actors together in the hierarchy, and drag and drop actors between folders.

-	Drag and drop actors on top of another actor to give them a parent-child relationship. When you move, rotate, or scale a parent actor, its children follow too.

You can have up to four Outliner panels active at any time. They all contain the same tree of actors, but you can choose to apply different filters to each.
